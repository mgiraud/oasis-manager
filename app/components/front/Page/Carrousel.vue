<template>
  <div class="overflow-hidden hidden md:flex flex-no-wrap flex-row bg-white" v-if="mediaObjects.length > 0">
...
<!--    </div>-->
    ...
    >
      <div v-for="imageObject in mediaObjects"
    ...
    <div @click="next" class="cursor-pointer grow-0 w-0 flex z-10 items-center" v-if="mediaObjects.length > 1">
...
const mediaObjects = computed(() => props.mediaNode.mediaObjects
  .filter(mediaObject => mediaObject.original === null)
  .map((mediaObject: MediaObject, index: number) => {
...
const current = ref(0)
const nbItems = mediaObjects.value.length
